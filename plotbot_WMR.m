...
aviobj = VideoWriter(['txt1.avi'],'Motion JPEG AVI'); % Declare an avi object
close all;
open(aviobj);
figure(index*3-2);
cla('reset');
...
for i=1:4:length(t)
    ...
    pause(0.05);     %Stop execution for 0.01 to make animation visible
        ...
        writeVideo(aviobj, frame); % Step 3: Add frame to avi object
end
close(aviobj);  % Close the avi object
hold off
% axis equal;
% disp(max(err1));
figure(index*3-1);
